...
class QuotationAttribution:
    ...
    def __init__(self, modelFile):
        ...
        state_dict = {
            ...
            if not k.startswith("bert.embeddings.position_ids")
        }
    ...
    def tag(self, quotes, entities, tokens):
        ...
        texts, metas, positions, global_entity_positions, quote_indexes = (
            self.get_representation(quotes, entities, tokens)
        )
    ...
    def get_representation(self, quotes, entities, tokens, doLowerCase=True):
        ...
        for q_id, (start_tok, end_tok) in enumerate(quotes):
            ...
            while (
                ...
                and count < window
                and wp_tok_count
                + len(self.model.tokenizer.tokenize(convert_word(tokens[start].text)))
                < 350
            ):
                if in_quotes[start] == 0:
                    count += 1
                    wp_tok_count += len(
                        self.model.tokenizer.tokenize(convert_word(tokens[start].text))
                    )
            ...
            while (
                ...
                and count < window
                and wp_tok_count
                + len(self.model.tokenizer.tokenize(convert_word(tokens[end].text)))
                < 475
            ):
                if in_quotes[end] == 0:
                    count += 1
                    wp_tok_count += len(
                        self.model.tokenizer.tokenize(convert_word(tokens[end].text))
                    )
                ...
                if entity_start in entities_by_start:
                    ...
                    if in_quotes[entity_start] == 0:
                        for entity_end in entities_by_start[entity_start]:
                            ...
                            if entity_end < end and entity_end + inserts[
                                entity_end - start
                            ] - start < len(toks):
                                # cand tuple values:
                                ...
                                cands.append(
                                    (
                                        min(
                                            abs(entity_end - start_tok),
                                            abs(entity_start - end_tok),
                                        ),
                                        entity_start
                                        + inserts[entity_start - start]
                                        - start,
                                        entity_end
                                        + inserts[entity_end - start]
                                        - start,
                                        None,
                                        "ENT",
                                        entity_start,
                        ...
                        abs_positions.append(
                            (
                                ...
                                " ".join(
                                    [
                                        t.text
                                        for t in tokens[global_start : global_end + 1]
                                    ]
                                ),
                            )
                        )
                            ...
                            (
                                ...
                                " ".join(
                                    [
                                        t.text
                                        for t in tokens[
                                            altquote_map[s][0] : altquote_map[s][1]
                                        ]
                                    ]
                                ),
                            )
