            ...
            for j in range(i + 1, len(parts)):
                ...
                for k in range(j + 1, len(parts)):
                    ...
                    for l in range(k + 1, len(parts)):
                        variants[
                            "%s %s %s %s" % (parts[i], parts[j], parts[k], parts[l])
                        ] = 1

                        for m in range(l + 1, len(parts)):
                            variants[
                                "%s %s %s %s %s"
                                % (parts[i], parts[j], parts[k], parts[l], parts[m])
                            ] = 1
    ...
    def cluster_only_nouns(self, entities, refs, tokens):
        ...
        for start, end, cat, text in entities:
            ...
            for i in range(start, end + 1):
                ...
                if (
                    hon_mapped is not None
                    or (tokens[i].pos == "NOUN" or tokens[i].pos == "PROPN")
                ) and tokens[i].text.lower()[0] != tokens[i].text[0]:
                    val = tokens[i].text
