    ...
    def __init__(self, model_params):
        ...
        with torch.no_grad():
            ...
            try:
                ...
                raise OSError(
                    f"You are trying to load the model {spacy_model}, but it is not installed. Please run \n python -m spacy download en_core_web_sm"
                )
            ...
            ref = (
                importlib_resources.files("booknlp.english.data") / "entity_cat.tagset"
            )
            ...
            with open(filename) as file:
                ...
                if self.doEvent or self.doEntities or self.doSS:
                    ...
                    if self.doSS:
                        ...
                        with open(
                            join(outFolder, "%s.supersense" % (idd)),
                            "w",
                            encoding="utf-8",
                        ) as out:
                            out.write(
                    ...
                    with open(
                        join(outFolder, "%s.tokens" % (idd)), "w", encoding="utf-8"
                    ) as out:
                        out.write(
                            "%s\n"
                            % "\t".join(
                ...
                if self.doQuoteAttrib:
                    ...
                    attributed_quotations = self.quote_attrib.tag(
                ...
                if self.doEntities:
                    ...
                    refs = self.name_resolver.cluster_narrator(
                        entities, in_quotes, tokens
                    )
                ...
                if self.doCoref:
                    ...
                    with open(
                        join(outFolder, "%s.book" % (idd)), "w", encoding="utf-8"
                    ) as out:
                ...
                if self.doEntities:
                    ...
                    with open(
                        join(outFolder, "%s.entities" % (idd)), "w", encoding="utf-8"
                    ) as out:
                ...
                if self.doQuoteAttrib:
                    with open(
                        join(outFolder, "%s.quotes" % (idd)), "w", encoding="utf-8"
                    ) as out:
                        out.write(
                            "\t".join(
                                [
                ...
                if self.doQuoteAttrib and self.doCoref:
                    ...
                    with open(
                        join(outFolder, "%s.book.html" % (idd)), "w", encoding="utf-8"
                    ) as out:
                        out.write("<html>")
                        out.write("""<head>
		  <meta charset="UTF-8">
                ...
                print(
                    ...
                    % (time.time() - originalTime, len(tokens))
                )
