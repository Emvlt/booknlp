...
class BERTSpeakerID(nn.Module):
    ...
    def __init__(self, base_model=None):
        ...
        self.tokenizer.add_tokens(
            ["[QUOTE]", "[ALTQUOTE]", "[PAR]", "[CAP]"], special_tokens=True
        )
    ...
    def get_batches(self, all_x, all_m, batch_size=32, doLowerCase=True):
        ...
        for i in range(0, len(all_x), batch_size):
            ...
            batches_m.append(
                {
                    "cands": torch.FloatTensor(np.array(current_batch_matrix_cands)).to(
                        device
                    ),
                    "quote": torch.FloatTensor(np.array(current_batch_matrix_quote)).to(
                        device
                    ),
                }
            )
    ...
    def evaluate(
        self, dev_x_batches, dev_m_batches, dev_y_batches, dev_o_batches, epoch
    ):
        ...
        with torch.no_grad():
            ...
            idd = 0
            for x1, m1, y1, o1 in zip(
                dev_x_batches, dev_m_batches, dev_y_batches, dev_o_batches
            ):
        ...
        print("Nones: %s" % nones)
        print(
            "Epoch %s, Quote F1: %.3f\tP: %.3f, R: %.3f" % (epoch, F, precision, recall)
        )
