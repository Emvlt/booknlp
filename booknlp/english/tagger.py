...
class Tagger(nn.Module):
    ...
    def forward_supersense(
        ...
        sequence_score1 = self.supersense_crf.score(
            torch.where(
                labels[0][:, 1:] == -100,
                torch.ones_like(labels[0][:, 1:]) * to_value,
                labels[0][:, 1:],
            ),
            lens[0] - 2,
            logits=tag_space1,
    ...
    def forward(
        ...
        sequence_score2 = self.crf.score(
            torch.where(
                labels[1][:, 1:] == -100,
                torch.ones_like(labels[1][:, 1:]) * to_value,
                labels[1][:, 1:],
            ),
            lens[1] - 2,
            logits=tag_space2,
        ...
        sequence_score3 = self.crf.score(
            torch.where(
                labels[2][:, 1:] == -100,
                torch.ones_like(labels[2][:, 1:]) * to_value,
                labels[2][:, 1:],
            ),
            lens[2] - 2,
            logits=tag_space3,
    ...
    def supersense_predict(
        self, wn, input_ids, attention_mask=None, transforms=None, lens=None
    ):
                ...
                for row in range(size[0]):
                    for col in range(size[1]):
                        if dev_batched_labels[b][row][col] != -100:
                            ...
                            all_golds.append(
                                dev_batched_labels[b][row][col].cpu().numpy()
                            )
            ...
            for b in range(len(batched_data)):
                ...
                for sentence in range(batch_len):
                    ...
                    for idx in range(1, len(batched_sents[b][sentence]) - 1):
                        ...
                        word_preds.append(
                            (batched_sents[b][sentence][idx], int(preds[sentence][idx]))
                        )
    ...
    def compress(self, labels, rev_tagset):
        ...
        for i in range(len(labels)):
            ...
            newkeep = []
            for j in keep:
                if labels[i][j] == -100 or not rev_tagset[labels[i][j]].startswith(
                    "I-"
                ):
                    newkeep.append(j)
