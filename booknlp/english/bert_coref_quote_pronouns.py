...
class BERTCorefTagger(nn.Module):
    ...
    def __init__(
        self, gender_cats, freeze_bert=False, base_model=None, pronominalCorefOnly=True
    ):
        ...
        self.tokenizer = BertTokenizer.from_pretrained(
            modelName, do_lower_case=False, do_basic_tokenize=False
        )
        ...
        self.mention_mention1 = nn.Linear(
            (3 * self.hidden_dim + 20 + 20) * 3 + 20 + 20 + 20 + 20, 150
        )
    ...
    def get_mention_reps(
        ...
        attention_weights = self.attention2(
            self.tanh(self.attention1(embeds))
        )  # num_sents x max_words x 1
        ...
        span_representation = torch.cat(
            (start_output, end_output, mentions, width_embeds, quote_embeds), 1
        )
        ...
        for idx, entity in enumerate(entities):
            ...
            if entity.in_quote:
                for j in range(len(entities)):
                    if (
                        not entities[j].in_quote
                        and entities[j].entity_id == entity.quote_eid
                    ):
    ...
    def is_compatible(
        self, cand_mention, eid, entity_properties, mention, ref_genders, score
    ):
        ...
        for inQuoteVal in [False, True]:
            ...
            for i in range(num_mentions):
                ...
                # (assign the mention a unique entity ID, to be clustered later)
                    ...
                    not doTrain and entity.proper != "PRON" and self.pronominalCorefOnly
                ):
                ...
                if i == 0:
                    ...
                    if not doTrain:
                        ...
                        if existing is None or (
                            existing is not None and existing[i] == -1
                        ):
                            assignment = curid
                            curid += 1
                            assignments[i] = assignment
                ...
                if len(cands_idx) == 0:
                    ...
                    for e in cands_idx:
                        if not entities[i].in_quote:
                            ...
                            if entities[i].quote_mention is None:
                                ...
                                attribution_assignment = assignments[
                                    entities[i].quote_mention
                                ]
                    ...
                    same_speaker_embeds = self.speaker_embeddings(
                        torch.LongTensor(same_speaker).to(device)
                    )
                    ...
                    nesteds_embeds = self.nested_embeddings(
                        torch.LongTensor(nest1).to(device)
                    )
                    nesteds_embeds2 = self.nested_embeddings(
                        torch.LongTensor(nest2).to(device)
                    )
                    ...
                    preds = self.mention_mention3(
                        self.tanh(
                            self.drop_layer_020(
                                self.mention_mention2(
                                    self.tanh(
                                        self.drop_layer_020(
                                            self.mention_mention1(concat)
                                        )
                                    )
                                )
                            )
                ...
                if doTrain:
                    ...
                    if len(truth[i]) == 0:
                        ...
                        golds = torch.index_select(
                            preds, 0, torch.LongTensor(truth[i]).to(device)
                        )
    ...
    def get_data(self, doc, ents, max_ents, max_words, batchsize=128):
        ...
        for idx, sent in enumerate(doc):
            matrix.append(
                self.get_matrix(
                    ents[idx], max_words_batch[batch_count], max_ents_batch[batch_count]
                )
            )
        ...
        with open(filename, encoding="utf-8") as file:
            ...
            for line in file:
                if line.startswith("#begin document"):
                    ...
                    docid = None
                    matcher = re.match(
                        "#begin document \((.*)\); part (.*)$", line.rstrip()
                    )
                    if matcher != None:
                    ...
                    (
                        ...
                        max_ents,
                        quote_antecedent_labels,
                    ) = self.get_ant_labels(all_doc_sents, all_doc_ents, all_doc_quotes)
                    ...
                    if docid in quotes and sid in quotes[docid]:
                        ...
                        for start_sid, start_wid, end_sid, end_wid, eid in quotes[
                            docid
                        ][sid]["END"]:
