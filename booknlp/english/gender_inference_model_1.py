                ...
                if idd != -1:
                    ...
                    for text in counts[idd]:
                        ...
                        for f in range(self.num_genders):
                            if (text, f) in self.joint_e_f_counts:
                                mf[f] = (
                                    self.joint_e_f_counts[text, f] * counts[idd][text]
                                )
                    ...
                    for text in counts[idd]:
                        ...
                        if first_name_token in self.honorific_priors:
                            for f in range(self.num_genders):
                                mf[f] = (
                                    self.honorific_priors[first_name_token][f]
                                    * counts[idd][text]
                                    * 10000
                                )
    ...
    def read_hyperparams(self, filename):
        ...
        with open(filename) as file:
            ...
            for honorific in self.honorific_priors:
                ...
                if total >= self.upper:
                    for i in range(self.num_genders):
                        self.honorific_priors[honorific][i] = (
                            self.honorific_priors[honorific][i] / total
                        ) * self.upper
    ...
    def tag(self, entities, tokens, refs, minThreshold=0):
        ...
        for epoch in range(self.num_epochs):
            ...
            if epoch < self.num_epochs - 1:
                self.delete_counts()
                self.add_hyperparameters_to_counts(
                    refs=refs, entities=entities, tokens=tokens
                )
...
if __name__ == "__main__":
    ...
    genderEM = GenderEM(
        ...
        hyperparameterFile=hyperparams,
        genders=genders,
    )
